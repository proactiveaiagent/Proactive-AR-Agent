...
from __init__ import data_list
...
def adapter_for_video(sample: Dict[str, Any], video_file: str):
    ...
    if "start_frame" in sample:
        ...
        if sample.get(
            ...
            current_observation_frame = sample[
        ...
        (
            ...
            frame_time,
            num_frames,
        ) = process_video_with_decord(video_file, data_args)
    return video, data_args.video_fps

def save_video_mp4_ffmpeg(
    frames,
    output_path,
    fps,
    crf=18,
    ...
    # 🔒 每个 worker 必须是独立路径（强烈建议）
    # output_path = f"{output_path}.{os.getpid()}.mp4"
    ...
    if ret != 0:
        raise RuntimeError(
            f"ffmpeg failed (code={ret}):\n{stderr}"
        )

# def save_video_mp4_ffmpeg(frames, output_path, fps):
...
def preprocess_qwen_visual(
    ...
    original_video_path = ""
    if "video" in source:
        original_video_path = source["video"]
        video_path = base_path / source["video"]
        video, fps = adapter_for_video(source, str(video_path))
        output_video_path = str(video_path).replace(".mp4", f"_processed_{index}.mp4")
        save_video_mp4_ffmpeg(video, output_video_path, fps)
        source["video"] = output_video_path.replace(str(base_path) + "/", "")
    ...
    try:
        ...
        raise SyntaxError(f"Processor failed at index {index} with error: {e}")
    finally:
        if 'video' in source:
            source['video'] = original_video_path
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_packed_item(self, sources, index) -> Dict[str, torch.Tensor]:

        if isinstance(sources, dict):
            if isinstance(source, dict):
                sources = [sources]
            assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
            return self._get_item(sources, index)
        ...
        if isinstance(sources, list):
            ...
            for i, source in enumerate(sources):
                ...
                data_list.append(self._get_item(source, str(index) + "_" + str(i)))
